...
import PaginationItemSkeleons from "@/components/common/PaginationItemSkeletons";
...
const NoticeContainer: React.FC<IProps> = ({ page, title, dataList, clickNoticeItem, onClickPageHandler }) => {
    return (
            ...
            <ContentsLayout>
                ...
                <PageNationLayout>
                    ...
                    {Array(Math.round(dataList.size / 4))
                        .fill(0)
                        .map((_, index) => (
                            <PageNumber isSelected={page === index + 1} onClick={() => onClickPageHandler(index + 1)}>
                                {index + 1}
                            </PageNumber>
                        ))}
                    <ArrowIcon
