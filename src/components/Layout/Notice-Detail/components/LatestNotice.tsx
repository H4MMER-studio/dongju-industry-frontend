...
const LatestNotice: React.FC<IProps> = ({ list, onClickGoToDetail, onClickGoToList }) => {
    return (
        <LatestNoticeLayout>
            <Title>최신글</Title>
            {list.map(({ _id, created_at, notice_title, notice_content, notice_images }) => (
                <ContentLayout key={_id} onClick={() => onClickGoToDetail(_id)}>
                    <LeftSide>
                        <DateText>{created_at.split("T")[0].replaceAll("-", ".")}</DateText>
                        <ContentTitle>{notice_title}</ContentTitle>
                        <ContentText>{parse(notice_content)}</ContentText>
                    </LeftSide>
                    {(notice_images?.length ?? 0) > 0 && (
                        <ContentImage alt={notice_images[0].name} src={notice_images[0].url} />
                    )}
                </ContentLayout>
            ))}
